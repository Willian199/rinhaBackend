...
WORKSPACE="$(pwd)/deps/gatling"
GATLING_BIN_DIR="${WORKSPACE}/bin"

echo "start"

sh ${GATLING_BIN_DIR}/gatling.bat -rm local -s RinhaBackendSimulation -rd "DESCRICAO" -rf ${WORKSPACE}/user-files/results -sf ${WORKSPACE}/user-files/simulations/rinhabackend -rsf ${WORKSPACE}/user-files/resources
